...
if (isset($_POST["submit"])) {
    $comments = $_POST["finding_comment"];
    $findings = $_FILES["finding"]["name"];
    $file_ext = pathinfo($findings, PATHINFO_EXTENSION);

    $filename = "file-" . date("YHism") . "." . $file_ext;
    $move_file = move_uploaded_file($_FILES["finding"]["tmp_name"], "exhibit/" . $filename);


    if ($move_file == TRUE) {

        $update_report = mysqli_query($conn, "update reports set emc_findings='$filename', emc_comment='$comments' where rid='$repid'") or die(mysqli_error($conn));
        if ($update_report) {
            $alert = "Response submitted successfully!";
            header("Refresh: 3;");
        } else {
            $alert = "Oops!looks like we are having a down time, please try after some minutes!";
        }
    } else {
        echo "<script>alert('Findings upload failed! pls try after some minutes');</script>";
    }
}
?>
                        ...
                        <div class="col-lg-4 col-md-6 col-12">
                            ...
                            <div class="box">
                                <div class="box-header with-border">
                                    <h3 class="box-title">Submit Findings</h3>
                                    <h5 style="color:greenyellow;"><?php echo $alert; ?></h5>
                                </div>
                                <!-- /.box-header -->
                                <div class="box-body">
                                    <?php
                                    if (($emc_comment == "" ) && ($emc_findings == "")) {
                                        ?>
                                        <form action="" method="post" enctype="multipart/form-data">
                                            <div class="form-group">
                                                <label>Executive Summary of Findings (FORMAT: .DOC, .DOCX, .pdf)</label>
                                                <input required="" name="finding" accept=".pdf, .doc, .docx" type="file" class="form-control" placeholder="Findings"/>
                                            </div>

                                            <div class="form-group">
                                                <label>Comment</label>
                                                <textarea name="finding_comment" class="form-control" rows="3" placeholder="Brief Comment of Findings"></textarea>
                                            </div>
                                            ...
                                            <div class="row box-body">
                                                ...
                                                <div class="col-xs-4">
                                                    <input type="submit" name="submit" value="SUBMIT" class="btn btn-primary btn-group-toggle btn-info btn-block btn-flat"> 
                                                </div><!-- /.col -->
                                            </div>
                                        </form> 
                                        <?php
                                    } else {
                                        echo "Findings Submitted";
                                    }
                                    ?>
                            ...
                            <div class="box">
                                ...
                                <div class="box-body">
                                    <form action="" method="post" enctype="multipart/form-data">
                                        ...
                                        <div class="form-group">
                                            <label>Chairman Resolution</label><br>
                                            <?php
                                        ...
                                        <div class="form-group">
                                            ...
                                            if ($report_status == "no") {
                                                ...
                                                ?>
                                                <input value="<?php echo $report_status; ?>" readonly="" name="email" type="email" class="form-control" placeholder="Pending"/>
                                                <?php
                                            }
                                            ?>
                                        </div>
