...
if (isset($_POST["submit"])) {
    $comments = $_POST["resolution_comment"];
    $findings = $_FILES["resolution"]["name"];
    $file_ext = pathinfo($findings, PATHINFO_EXTENSION);

    $filename = "file-" . date("YHism") . "." . $file_ext;
    $move_file = move_uploaded_file($_FILES["resolution"]["tmp_name"], "exhibit/" . $filename);


    if ($move_file == TRUE) {

        $update_report = mysqli_query($conn, "update reports set chairman_resolution='$filename', chm_comment='$comments', status='yes' where rid='$repid'") or die(mysqli_error($conn));
        if ($update_report) {
                        ...
                        <div class="col-lg-4 col-md-6 col-12">
                            ...
                            <div class="box">
                                <div class="box-header with-border">
                                    <h3 class="box-title">Submit Final Resolution</h3>
                                    <h5 style="color:greenyellow;"><?php echo $alert; ?></h5>
                                </div>
                                <!-- /.box-header -->
                                <div class="box-body">
                                    <?php
                                    if (($chairmain_comment == "" ) && ($chairmain_resolution_file == "")) {
                                        ?>
                                        <form action="" method="post" enctype="multipart/form-data">
                                            <div class="form-group">
                                                <label>Final Resolution (FORMAT: .DOC, .DOCX, .pdf)</label>
                                                <input required="" name="resolution" accept=".pdf, .doc, .docx" type="file" class="form-control" placeholder="Findings"/>
                                            </div>

                                            <div class="form-group">
                                                <label>Comment</label>
                                                <textarea name="resolution_comment" class="form-control" rows="3" placeholder="Brief Comment of resolution"></textarea>
                                            </div>
...
<hr>
                                        <div class="form-group">
                                            <label><b>Committee Findings</b></label><br>
                                            <?php
                                            if ($emc_findings == "") {
                                                echo "PENDING...";
                                            } else {
